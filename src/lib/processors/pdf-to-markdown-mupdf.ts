...
export async function pdfToMarkdownMupdf(ctx: ProcessorContext): Promise<ProcessorResult> {
  const { document } = ctx;
  ...
  if (document.sourceType !== 'upload') {
    return { 
      success: false, 
      error: 'PDF to Markdown (MuPDF) processor only works with uploaded documents' 
    };
  }
  ...
  try {
    ...
    const response = await fetch(`${DOCUMENT_CONVERTER_API_URL}/convert-mupdf`, {
      method: 'POST',
      headers: {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Failed to convert PDF with MuPDF',
    };
  }
}
