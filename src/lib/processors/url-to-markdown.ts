...
export async function urlToMarkdown(ctx: ProcessorContext): Promise<ProcessorResult> {
  ...
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : 'Failed to convert HTML to Markdown',
    };
  }
}
