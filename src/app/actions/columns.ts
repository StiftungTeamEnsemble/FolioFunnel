...
const createColumnSchema = z.object({
  ...
  processorType: z.enum(['pdf_to_markdown_mupdf', 'pdf_to_metadata', 'pdf_to_thumbnail_mupdf', 'url_to_text', 'chunk_text', 'create_embeddings', 'openai_transform', 'count_tokens']).optional(),
...
export async function createColumn(projectId: string, formData: FormData) {
  ...
  const processorType = formData.get('processorType') as ProcessorType | undefined;
  const processorConfigStr = formData.get('processorConfig') as string | undefined;
  ...
  try {
    ...
    const column = await prisma.column.create({
      data: {
        ...
        processorType: mode === 'processor' ? processorType as ProcessorType : null,
